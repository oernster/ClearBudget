...
def _card(apr: float | None = 24.9, balance_pence: int = 10000,
          min_pct: float | None = None) -> CreditCard:
    return CreditCard(
        id=2,
        name="TestCard",
...
def _bill(*, name: str, pence: int, payment_method_id: int = 1,
          category: str = "groceries", target_card_id: int | None = None,
          day: int | None = None) -> Bill:
    return Bill(
        id=1, name=name, amount=Amount(pence=pence),
        payment_method_id=payment_method_id, category=category,
        bill_type="fixed", day_of_month=day,
        start_ym=YearMonth(2000, 1), end_ym=None, active=True,
...
class TestCalculateCardMonthlyState:
    def test_charges_on_card(self) -> None:
        ...
        state = calculate_card_monthly_state(card=card, opening_balance_pence=5000, bills=bills)
        assert state.charges.pence == 1299

    def test_payment_received(self) -> None:
        card = _card(apr=0.0, balance_pence=10000)
        bills = [_bill(name="Card Pmt", pence=5000, category="credit_payment", target_card_id=card.id)]
        state = calculate_card_monthly_state(card=card, opening_balance_pence=10000, bills=bills)
        assert state.payment_received.pence == 5000

    def test_monthly_interest_zero_apr(self) -> None:
        card = _card(apr=0.0, balance_pence=10000)
        state = calculate_card_monthly_state(card=card, opening_balance_pence=10000, bills=[])
    ...
    def test_closing_balance(self) -> None:
        ...
        bills = [
            ...
            _bill(name="Pmt", pence=3000, category="credit_payment", target_card_id=card.id),
        ]
        state = calculate_card_monthly_state(card=card, opening_balance_pence=10000, bills=bills)
    ...
    def test_closing_balance_floored_at_zero(self) -> None:
        ...
        bills = [_bill(name="Overpay", pence=9999, category="credit_payment", target_card_id=card.id)]
        state = calculate_card_monthly_state(card=card, opening_balance_pence=0, bills=bills)
        assert state.closing_balance.pence == 0

    def test_payment_date_extracted_from_bill(self) -> None:
        card = _card(apr=0.0)
        bills = [_bill(name="Pmt", pence=500, category="credit_payment", target_card_id=card.id, day=22)]
        state = calculate_card_monthly_state(card=card, opening_balance_pence=5000, bills=bills)
        assert state.payment_date == 22

    def test_payment_date_none_when_no_payment_bill(self) -> None:
        card = _card(apr=0.0)
        state = calculate_card_monthly_state(card=card, opening_balance_pence=5000, bills=[])
    ...
    def test_minimum_payment_floor_25(self) -> None:
        ...
        state = calculate_card_monthly_state(card=card, opening_balance_pence=100, bills=[])
    ...
    def test_minimum_payment_interest_plus_1_percent(self) -> None:
        ...
        state = calculate_card_monthly_state(card=card, opening_balance_pence=200000, bills=[])
    ...
    def test_minimum_payment_uses_per_card_percent_when_set(self) -> None:
        ...
        state = calculate_card_monthly_state(card=card, opening_balance_pence=112698, bills=[])
    ...
    def test_payment_targeting_other_card_not_counted(self) -> None:
        card = _card(apr=0.0)
        bills = [_bill(name="Pmt", pence=5000, category="credit_payment", target_card_id=card.id + 99)]
        state = calculate_card_monthly_state(card=card, opening_balance_pence=10000, bills=bills)
