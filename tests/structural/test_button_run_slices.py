...
Every view builds its own copy of the strip as `self.view_btns`, whose ring
...
naming a single button (`pre_theme=(self.view_btns[-1],)` puts Archive in the
...
The repair is `view_buttons.stops_before`, which inserts into the whole run
...
_RUN_ATTRIBUTE = "view_btns"
_DECLARATION = "nav_targets"
# The run as drawn, plus Archive off the end.
_ALLOWED = frozenset({"self.view_btns[:-1]", "self.view_btns[-1:]"})


def _button_run_slices(tree: ast.Module) -> list[tuple[int, str]]:
    """Every `<x>.view_btns[...]` subscript inside a `nav_targets` body."""
    found = []
    declarations = [
        node
...
class TestTheButtonRunIsNeverCutUp:
    ...
    def test_only_the_whole_run_and_archive_are_sliced(self):
        ...
        for path in sorted(_UI.rglob("*.py")):
            ...
            for lineno, written in _button_run_slices(tree):
        ...
        assert not offenders, (
            ...
            "else. Take the run whole and use `view_buttons.stops_before` to put "
            "anything into it.\n" + "\n".join(offenders)
        )
