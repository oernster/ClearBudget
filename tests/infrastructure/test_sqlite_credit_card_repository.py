...
class TestInterestRatePersistence:
    ...
    def test_apr_null_when_zero(self, db) -> None:
        ...
        c = CreditCard(
            id=0, name="ZeroAPR", credit_limit=Amount(pence=100000),
            current_balance_used=Amount(pence=0), interest_rate_apr=None,
            payment_due_day=1, active=1,
        )
    ...
    def test_expiry_date_saved_on_add(self, db) -> None:
        repo = _repo(db)
        card = CreditCard(
            id=0, name="ExpiryCard", credit_limit=Amount(pence=100000),
            current_balance_used=Amount(pence=0), interest_rate_apr=24.9,
            payment_due_day=1, card_expiry_month=6, card_expiry_year=2028, active=1,
        )
    ...
    def test_expiry_date_updated(self, db) -> None:
        ...
        updated = CreditCard(
            id=saved.id, name=saved.name, credit_limit=saved.credit_limit,
            current_balance_used=saved.current_balance_used,
            interest_rate_apr=saved.interest_rate_apr,
            payment_due_day=saved.payment_due_day,
            card_expiry_month=9, card_expiry_year=2029,
            minimum_payment_pence=saved.minimum_payment_pence,
            minimum_payment_percent=saved.minimum_payment_percent,
            active=saved.active,
        )
