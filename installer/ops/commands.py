...
Every external command the setup program runs (today, the PowerShell helper
that removes the install directory once the uninstaller has exited) goes
through a ``CommandRunner``. Production code uses the real runner below; tests
pass a hand-written fake, so no test ever spawns a process it did not intend
to. British spelling is used in comments.
"""
...
_POWERSHELL_FLAGS = ("-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass")
...
class SubprocessRunner:
    ...
    def run(self, args: Sequence[str], *, timeout: float) -> CommandResult:
        """Run a command; a failure to start is reported, not raised."""
        try:
