...
def calculate_card_monthly_state(
    *,
    ...
    closing_pence = opening_balance_pence + charges_pence - payment_pence + monthly_interest_pence
    ...
    if opening_balance_pence > 0:
        if card.minimum_payment_pence is not None:
            ...
            minimum_pence = max(1, int(opening_balance_pence * card.minimum_payment_percent / 100))
        else:
            # Generic fallback: max(£25, interest + 1% of balance)
            one_percent_pence = int(opening_balance_pence * 0.01)
            minimum_pence = max(_MIN_PAYMENT_FLOOR_PENCE, monthly_interest_pence + one_percent_pence)
    else:
        minimum_pence = 0

    payment_bill = next(
        (b for b in bills if b.category == "credit_payment" and b.target_card_id == card.id),
        None,
    )
