...
port, so it never depends on the network or on GitHub's payload shape. A
source that cannot reach the network returns None rather than raising, keeping
the update check non-blocking and silent on failure.
...
if TYPE_CHECKING:
    from clear_budget.application.dto.update_info import ReleaseInfo
