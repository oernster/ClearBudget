...
example "v4.2.0"). A version that cannot be parsed as dotted integers is
treated as not-newer, so a malformed tag can never trigger a spurious update
prompt.

This is application-layer logic with no domain meaning of its own and no I/O,
so it lives beside the update service that uses it.
"""
...
# Optional release-tag prefix stripped before parsing (for example "v4.2.0").
_TAG_PREFIX = "v"
