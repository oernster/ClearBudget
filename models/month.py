...
class Month:
    ...
    def get_or_create(db, year_month):
        ...
        for row in cursor.fetchall():
            db.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, 0)
            ''', (month_id, row['id'], row['name'], row['amount'], row['payment_method_id'], row['category'], row['day_of_month']))
        ...
        for row in cursor.fetchall():
            db.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (month_id, row['id'], row['name'], row['amount'], row['is_reliable'], row['day_of_month']))
    ...
    def get_month_data(db, year_month):
        ...
        cursor = db.execute('SELECT * FROM month_bills WHERE month_id = ? ORDER BY day_of_month, name', (month_id,))
        bills = cursor.fetchall()

        cursor = db.execute('SELECT * FROM month_income WHERE month_id = ? ORDER BY day_of_month', (month_id,))
    ...
    def archive(db, year_month):
        ...
        db.execute('UPDATE months SET is_archived = 1 WHERE year_month = ?', (year_month,))
        db.commit()

    @staticmethod
    def unarchive(db, year_month):
        """Mark month as active."""
        db.execute('UPDATE months SET is_archived = 0 WHERE year_month = ?', (year_month,))
        db.commit()
